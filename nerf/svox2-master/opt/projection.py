...
with torch.no_grad():
    ...
    for img_id in tqdm(range(0,n_images)):        #tqdm(range(0, n_images, img_eval_interval)):  n_images
        ...
        seg_img_bw=dset.inverse_bw_results[img_id]
        ...
        rays = cam.gen_rays()
        batch_size=5000
        seg_flat=seg_img_bw.flatten()
        #print("seg_flat is on the device ",seg_flat.device)
        #seggrid=grid.seg_voxel(rays[0:batch_size],return_raylen=args.ray_len,segimg=seg_flat[0:batch_size],seggrid=seggrid)
           
    
        for batch_start in range(0, cam.height * cam.width, batch_size):
            seggrid=grid.seg_voxel(rays[batch_start:batch_start+batch_size],return_raylen=args.ray_len,segimg=seg_flat[batch_start:batch_start+batch_size],seggrid=seggrid)
            #print(batch_start/(cam.height * cam.width))
            round_float=float(img_id)/float(n_images)
            process=((batch_start/(cam.height * cam.width))/n_images+round_float)*100
            
            print("About %.2f %% is calculated" % process)
            
        print("the sum of the segimg is ",torch.sum(seg_flat))
        print("the sum of the current seggrid is ",torch.sum(seggrid))
        file_name=str(img_id)+"_inverse.pt"
        save_path=path.join(args.seg_results_save_name,file_name)
        torch.save(seggrid,save_path)
        
        print("the sum of the current seggrid is ",torch.sum(seggrid))
        print("segmation results are saved in {} ".format(save_path))
        '''
        single_ray=cam.gen_one_ray(device,xcor,ycor)
        starttime=time.time()
        rgb_pred_test = grid.volume_render_image_test(cam, use_kernel=False)
        endtime=time.time()
        #print("generate one ray costs ",endtime-starttime)
        print("render a image costs ",endtime-starttime)
        #starttime=time.time()
        
        vlink,slink=grid.find_voxel(single_ray,return_raylen=args.ray_len)
        color=grid._volume_render_gradcheck_nvol_lerp(single_ray,return_raylen=args.ray_len)
        #endtime=time.time()
        #print("find voxel costs ",endtime-starttime)
        #print("the voxel link here is ",vlink)
        #print("the space link here is ",slink)
