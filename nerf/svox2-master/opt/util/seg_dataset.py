# Complement of the origin dataset and loader
...
class SegDataset(DatasetBase):
    def __init__(
        ...
        **kwargs
        ...
        all_rgb_result=[]
        all_bw_result=[]
        seg_root=path.join(root,"2Dsegmentation")
        
        split_name = split if split != "test_train" else "train"

        print("LOAD SEG DATA", root, 'split', split)
        ...
        def sort_key(x):
            if len(x) > 2 and x[1] == "_":
                return x[2:]
            return x
        def look_for_seg_dir(cands, required=True):
            for cand in cands:
                if path.isdir(path.join(seg_root, cand)):
                    return cand
            if required:
        ...
        result_rgb_dir_name = look_for_seg_dir(["result_rgb"])  
        result_bw_dir_name = look_for_seg_dir(["result"])
        img_dir_name = look_for_seg_dir(["rgb"])
        
        control_box_dir_name = look_for_seg_dir(["control_box"])
        
        result_rgb_files = sorted(os.listdir(path.join(seg_root, result_rgb_dir_name)), key=sort_key)
        result_bw_files = sorted(os.listdir(path.join(seg_root, result_bw_dir_name)), key=sort_key)
        img_files=sorted(os.listdir(path.join(seg_root, img_dir_name)), key=sort_key)
        
        
        assert len(result_rgb_files) > 0, "No matching images in directory: " + path.join(seg_root, result_rgb_dir_name)
        self.result_rgb_files =  result_rgb_files
        self.bw_files=result_bw_files
        self.img_files = img_files
        
        
        dynamic_resize = scale < 1
        self.use_integral_scaling = False
        for img_fname in tqdm(result_rgb_files):
            img_path = path.join(seg_root, result_bw_dir_name, img_fname)
            result_bw = imageio.imread(img_path,as_gray=True)
            print(type(result_bw))
            print(result_bw.shape)
            #print(result_bw)
            img_path = path.join(seg_root, result_rgb_dir_name, img_fname)
            result_rgb = imageio.imread(img_path)
            img_path = path.join(seg_root, img_dir_name, img_fname)
            image = imageio.imread(img_path)
            pose_fname = path.splitext(img_fname)[0] + ".txt"
            pose_path = path.join(root, pose_dir_name, pose_fname)
            box_fname = path.splitext(img_fname)[0] + ".txt"
            box_path = path.join(seg_root, control_box_dir_name, box_fname)
            
            #  intrin_path = path.join(root, intrin_dir_name, pose_fname)

            cam_mtx = np.loadtxt(pose_path).reshape(-1, 4)
            box_mtx= np.loadtxt(box_path).reshape(-1, 2)
            if len(cam_mtx) == 3:
            ...
            all_box.append(torch.from_numpy(box_mtx))  # control_box (2,2) OpenCV 
            ...
            all_rgb_result.append(torch.from_numpy(result_rgb))
            all_bw_result.append(torch.from_numpy(result_bw))
        ...
        self.rgb_results=torch.stack(all_rgb_result).double() / 255.0
        self.bw_results=torch.stack(all_bw_result).double() / 255.0
        ...
        if self.rgb_results.size(-1) == 4:
            if white_bkgd:
                # Apply alpha channel
                self.rgb_results = self.rgb_results[..., :3] * self.rgb_results[..., 3:] + (1.0 - self.rgb_results[..., 3:])
            else:
                self.rgb_results = self.rgb_results[..., :3]
        self.rgb_results = self.rgb_results.float()
        
        self.bw_results= self.bw_results.float()
        self.inverse_bw_results=1-self.bw_results
