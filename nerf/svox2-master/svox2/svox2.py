...
class SparseGrid(nn.Module):
    ...
    def _seg_render_gradcheck_nvol_lerp(self, rays: Rays,segimg,seg_grid, return_raylen: bool=False):
        """
        trilerp gradcheck version
        """
        origins = self.world2grid(rays.origins)
    ...
    def seg_voxel(self, rays: Rays,segimg,seggrid,return_raylen: bool=False):
        """
        Find the projecting grid
        return: 
        """
        print("start to find the projecting grid")
        reso=self.shape[0]
        ...
        while good_indices.numel() > 0:
            ...
            #print("the shape of the l is ",l.shape)
            # BEGIN CRAZY TRILERP
            lx, ly, lz = l.unbind(-1)
            links000 = self.links[lx, ly, lz]
            links001 = self.links[lx, ly, lz + 1]
            ...
            #print("the shape of the rgb size is",rgb.shape)
            
            
            out_rgb[good_indices] += rgb
            #print("the shape of the rgb size is",out_rgb.shape)
            #print("the shape of the good_incices size is",out_rgb[good_indices].shape)
            #print(out_rgb.shape)
            t += self.opt.step_size
        ...
        out_rgb_sum=torch.sum(out_rgb,dim=1)
        
        
        one=torch.ones(out_rgb_sum.shape,dtype=torch.float,device=my_device)
        
        out_rgb_sum=torch.where(out_rgb_sum>1e-9,out_rgb_sum,one)
        
        
        #print("the shape of the out rgb sum is",out_rgb_sum.shape)
        #print("second round iteration")
        origins = self.world2grid(rays.origins)
        dirs = rays.dirs / torch.norm(rays.dirs, dim=-1, keepdim=True)
        viewdirs = dirs
        B = dirs.size(0)
        ...
        while good_indices.numel() > 0:
            ...
            #print("rgb sum is",torch.sum(rgb))
            part_rgb_sum=torch.sum(rgb,dim=1)
            #print("part shape is",part_rgb_sum.shape)
            
            zero=torch.zeros(part_rgb_sum.shape,dtype=torch.float,device=my_device)
            #print("place 1 part rgb sum is",torch.sum(part_rgb_sum))
            part_rgb_sum=torch.where(part_rgb_sum>1e-9,part_rgb_sum,zero)
            
            #print("part rgb sum is",torch.sum(part_rgb_sum))
            ...
            #a=seggrid[lx,ly,lz]
            #print(a.shape)
            m=reso*reso*lx + reso*ly+lz  
            #values = torch.autograd.Variable(torch.mul(torch.div(part_rgb_sum,out_rgb_sum[good_indices]),segimg[good_indices])).expand_as(m)
            values=torch.mul(torch.div(part_rgb_sum,out_rgb_sum[good_indices]),segimg[good_indices])
            seggrid.put_(m, values, accumulate=True)
            #seggrid[lx,ly,lz]+= torch.mul(torch.div(part_rgb_sum,out_rgb_sum[good_indices]),segimg[good_indices])
            img_sum+=torch.sum(segimg[good_indices])
            seggrid_sum=torch.sum(seggrid)
            total_sum+=torch.sum(torch.mul(torch.div(part_rgb_sum,out_rgb_sum[good_indices]),segimg[good_indices]))
            #print(rgb.shape)
            
            #if(iter%100==0):
                #print("About %d iterations times",iter)
            #    print("About {} iterations time in total".format(iter))
        ...
        #print("image sum is ",img_sum)
        #print("seggrid sum is ",seggrid_sum)
        #print("One iteration in project cost ",time_max)
        #print("Numer of iteration times in projection is",iter)
        return seggrid
    
    def seg_voxel_mask(self, rays: Rays,segimg,seggrid,return_raylen: bool=False):
        """
        Find the projecting grid
        return: 
        """
        ...
        while good_indices.numel() > 0:
            ...
            m=reso*reso*lx + reso*ly+lz  
            #values = torch.autograd.Variable(torch.mul(torch.div(part_rgb_sum,out_rgb_sum[good_indices]),segimg[good_indices])).expand_as(m)
            values=torch.max(torch.zeros(segimg[good_indices].shape),segimg[good_indices])
            seggrid.put_(m, values, accumulate=True)
            #seggrid[lx,ly,lz]+= torch.mul(torch.div(part_rgb_sum,out_rgb_sum[good_indices]),segimg[good_indices])
    ...
    def seg_render(
        self, rays: Rays,  segimg, seg_grid,use_kernel: bool = True, randomize: bool = False,
        return_raylen: bool=False
    ):
        """
        Standard volume rendering. See grid.opt.* (RenderOptions) for configs.
        ...
        warn("Using slow volume rendering, should only be used for debugging")
        return self._seg_render_gradcheck_nvol_lerp(rays,segimg=segimg,seg_grid=seg_grid, return_raylen=return_raylen)
        '''
        if self.opt.backend == 'nvol':
            return self._seg_render_gradcheck_nvol_lerp(rays,segimg=segimg,seg_grid=seg_grid, return_raylen=return_raylen)
        else:
            print("wrong in line 1374")
            return self._volume_render_gradcheck_lerp(rays, return_raylen=return_raylen)
        '''
    ...
    def project_seg_grid(
        self, camera: Camera,segimg, use_kernel: bool = True, randomize: bool = False,
        batch_size : int = 5000,
        return_raylen: bool=False,
        reso: int=128, 
    ):
        ...
        seg_grid= np.zeros((reso,reso))
        print(type(segimg))
        
        
        for batch_start in range(0, camera.height * camera.width, batch_size):
            seg_grid = self.seg_render(rays[batch_start:batch_start+batch_size],
                                                use_kernel=use_kernel,
                                                randomize=randomize,
                                                return_raylen=return_raylen,
                                                segimg=segimg,
                                                seg_grid=seg_grid)
            #all_rgb_out.append(rgb_out_part)
            #all_seg_out.append(seg_grid_part)
    ...
    def sparsify_background(
        ...
        data_mask = self.backgroundound_data[..., -1] >= sigma_thresh
